...
local book = {
	...
	create = function(self, player, _title, _author, content)
		...
		if not _title or helper.string.is_empty(_title) then
			_title = S("No title")
		end
		if not _author or helper.string.is_empty(_author) then
			_author = player_name
		end
	...
	make_formspec = function(self, focus)
		...
		if helper.bool.check(self.writable) then
			writable = true
		end
		if helper.bool.check(self.signed) then
			signed = true
		end
	...
	play_sound = function(self, _type)
		...
		if soundfile then
			sound.play("player", minetest.get_player_by_name(self.user_name), soundfile)
		end
	end,
...
minetest.register_craftitem(modname..":book", {
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		local mybook
		if helper.string.is_empty(meta:get_string(modname..":book")) then
