...
local function remove_viewfinder(player, hud_id)
	if hud_id then
		player:hud_remove(hud_id)
	end
end

function bowz.spawn_arrow(user, strength, itemstack)
	local pos = user:get_pos()
	pos.y = pos.y + 1.5 -- camera offset
	local dir = user:get_look_dir()
	local yaw = user:get_look_horizontal()
	local meta = itemstack:get_meta()
	local arrow = meta:get_string(modpath..":".."charged_arrow")
	local obj = nil
...
function bowz.launch_arrow(user, name, def, itemstack)
	if not bowz.spawn_arrow(user, def.strength, itemstack) then --throw arrow (spawn arrow entity)
		return -- something failed
	end
	if def.sounds then
		local user_pos = user:get_pos()
		if not def.sounds.soundfile_fire_arrow then
			def.sounds.soundfile_fire_arrow = "bowz_fire_arrow"
		end
		bowz.make_sound("pos", user_pos, def.sounds.soundfile_fire_arrow, DEFAULT_GAIN, DEFAULT_MAX_HEAR_DISTANCE)
	end
	...
	if def.viewfinder then --reset the viewfinder-zoom
		if not(user:get_fov() == 0) then
			user:set_fov(0)
			remove_viewfinder(user, itemstack:get_meta():get_int(modpath..":".."viewfinder_id"))
		end
	end
	return itemstack
end
...
function bowz.register_bow(name, def)
		...
		if not inv:remove_item("main", inventory_arrow):is_empty() then
			minetest.after(def.charge_time or 0, function(v_user, v_name)
				...
				if wielded_item_name == v_name then
					...
					meta:set_string(modpath..":".."charged_arrow", arrow) --save the arrow in the meta
			...
			if def.sounds then
				...
				if not def.sounds.soundfile_draw_bow then
					def.sounds.soundfile_draw_bow = "bowz_draw_bow"
				end
				bowz.make_sound("pos", user_pos, def.sounds.soundfile_draw_bow, DEFAULT_GAIN, DEFAULT_MAX_HEAR_DISTANCE)
			end
			return itemstack
		end
	end
	...
	minetest.register_tool(charged_name, {
		...
		on_use = function(itemstack, user, pointed_thing)
			return bowz.launch_arrow(user, name, def, itemstack)
		end,
			...
			if user:get_fov() == 0 then
				...
				if def.viewfinder.texture then
					...
					if viewfinder_texture == "" then
						viewfinder_texture = "bowz_viewfinder.png"
					end
					itemstack:get_meta():set_int(modpath..":".."viewfinder_id", show_viewfinder(user, viewfinder_texture))
				end
			else
				user:set_fov(0)
				if def.viewfinder.texture then
					remove_viewfinder(user, itemstack:get_meta():get_int( "bowz:viewfinder_id"))
				end
			end
			return itemstack
		end
...
function bowz.register_arrow(name, def)
	minetest.register_entity(name, {
		...
		on_step = function(self, dtime)
			...
			if def.sounds and not(def.sounds.soundfile_hit_arrow) then
				def.sounds.soundfile_hit_arrow = "bowz_hit_arrow"
			end
				...
				if thing.type == "object" and thing.ref ~= self.object then
					if not thing.ref:is_player() or thing.ref:get_player_name() ~= self.shooter_name then
						...
						if def.sounds then
							...
							if thing_pos then
								bowz.make_sound("pos", thing_pos, def.sounds.soundfile_hit_arrow, DEFAULT_GAIN, DEFAULT_MAX_HEAR_DISTANCE)
							end
						end
						...
						bowz.boom_effect(def, pos) -- BOOM
						bowz.water_effect(def, pos) -- water - extinguish fires
				...
				elseif thing.type == "node" then
					...
					if drawtype == 'liquid' then
						if not self.liquidflag then
							...
							bowz.splash(self.old_pos, "bubble.png")
						end
					...
					if minetest.registered_items[node_name].walkable then
						if not(def.embed) then
							...
							if thing.under then
								...
								node_above_drawtype = minetest.registered_nodes[minetest.get_node(pos_above).name].drawtype
									or "normal"
						...
						if def.sounds then
							if pos then
								bowz.make_sound("pos", pos, def.sounds.soundfile_hit_arrow,
									DEFAULT_GAIN, DEFAULT_MAX_HEAR_DISTANCE)
							end
						end
			...
			if def.effects and def.effects.trail_particle then
				bowz.trail(self.old_pos, pos, def.effects.trail_particle)
			end
			self.old_pos = pos
		end,
	})
...
function bowz.make_sound(dest_type, dest, soundfile, gain, max_hear_distance)
...
function bowz.boom_effect(def, pos)
	if def.effects.explosion and def.effects.explosion.mod then
		...
		if minetest.get_modpath(mod_name) ~= nil then
			if mod_name == "boomz" then
				boomz.boom(pos, def.effects.explosion.radius, def.effects.explosion.damage)
			end
		end
	end
end

function bowz.water_effect(def, pos)
	if def.effects.water then
		if def.effects.water.particles then
			bowz.splash(pos, "bowz_water.png")
		end
		...
		if flames and #flames > 0 then
			for i=1, #flames do
				...
				if def.effects.water.particles then
					bowz.splash(flames[i], "bowz_water.png")
				end
			end
		end
	end
...
function bowz.trail(old_pos, pos, trail_particle)
...
function bowz.splash(old_pos, splash_particle)
	minetest.add_particlespawner({
		amount = 5,
		time = 1,
		minpos = old_pos,
