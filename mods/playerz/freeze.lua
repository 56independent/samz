function playerz.is_frozen(player)
	return minetest.is_yes(player:get_meta():get_string("playerz:frozen"))
end

minetest.register_entity("playerz:freeze", {
	...
	on_step = function(self, dtime)
		...
		if not player or not playerz.is_frozen(player) then
			self.object:remove()
			return
		end
	end,
...
function playerz.freeze(player, pos)
	player:get_meta():set_string("playerz:frozen", "true")
	...
	local obj = minetest.add_entity(pos, "playerz:freeze")
	obj:get_luaentity():set_frozen_player(player)
end

function playerz.unfreeze(player)
	player:get_meta():set_string("playerz:frozen", "")
