...
screwz.ROTATE_FACE = 1
screwz.ROTATE_AXIS = 2
screwz.disallow = function(pos, node, user, mode, new_param2)
	return false
end
screwz.rotate_simple = function(pos, node, user, mode, new_param2)
	if mode ~= screwz.ROTATE_FACE then
		return false
	end
end
...
screwz.rotate.facedir = function(pos, node, mode)
...
screwz.rotate.colorfacedir = screwz.rotate.facedir

local wallmounted_tbl = {
	[screwz.ROTATE_FACE] = {[2] = 5, [3] = 4, [4] = 2, [5] = 3, [1] = 0, [0] = 1},
	[screwz.ROTATE_AXIS] = {[2] = 5, [3] = 4, [4] = 2, [5] = 1, [1] = 0, [0] = 3}
}

screwz.rotate.wallmounted = function(pos, node, mode)
...
screwz.rotate.colorwallmounted = screwz.rotate.wallmounted

-- Handles rotation
screwz.handler = function(itemstack, user, pointed_thing, mode, uses)
	...
	local fn = screwz.rotate[ndef.paramtype2]
...
-- Screwdriver
minetest.register_tool("screwz:screwdriver", {
	description = S("Screwdriver") .. "\n" .. S("(left-click rotates face, right-click rotates axis)"),
	inventory_image = "screwdriver.png",
	groups = {tool = 1},
	on_use = function(itemstack, user, pointed_thing)
		screwz.handler(itemstack, user, pointed_thing, screwz.ROTATE_FACE, 200)
		return itemstack
	end,
	on_place = function(itemstack, user, pointed_thing)
		screwz.handler(itemstack, user, pointed_thing, screwz.ROTATE_AXIS, 200)
		return itemstack
	end,
})

minetest.register_craft({
	output = "screwz:screwdriver",
	type = "shaped",
	recipe = {
		{"nodez:iron_ingot", ""},
		{"", "group:stick"}
